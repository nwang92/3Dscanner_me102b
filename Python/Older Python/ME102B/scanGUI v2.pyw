...
class pleaseWait1(wx.MessageDialog):
    ...
    def __init__(self):
        """Constructor"""
        wx.MessageDialog.__init__(self, None, 'Please wait while finishing cycle/resetting the sensor array...', 'Info', wx.OK)

class pleaseWait2(wx.MessageDialog):
 
    def __init__(self):
        """Constructor"""
        wx.MessageDialog.__init__(self, None, 'Please wait while generating STL file...', 'Info', wx.OK)
...
class scanStopDialog(wx.Dialog):
 
    def __init__(self):
        """Constructor"""
        wx.Dialog.__init__(self, None, title="Stop Scan", pos=(1100,400), size=(150, 75))
        self.InitUI()  
    ...
    def InitUI(self):
        ...
        stopbutt = wx.Button(panel, wx.ID_CLOSE, "Stop Scan", size=(125,50))
        stopbutt.SetFont(wx.Font(15, wx.SWISS, wx.NORMAL, wx.NORMAL))
        stopbutt.SetBackgroundColour('#F38630')
        stopbutt.Bind(wx.EVT_BUTTON, self.checkDone)
        butt.Add(stopbutt)
        box.Add(butt, wx.ALIGN_CENTER | wx.EXPAND, 0)
        self.ShowWithEffect(wx.SHOW_EFFECT_BLEND)
        
    def checkDone(self, e):
        global checkDone
        checkDone = 1
